...
class Four_Rooms_Environment(gym.Env):
    ...
    def __init__(self, grid_width=13, grid_height=13, stochastic_actions_probability=1.0/3.0, random_start_user_place=True,
                 random_goal_place=True, pixel=False, level=1):
        ...
        self.step_reward_for_not_achieving_goal = -0.1
        self.state_only_dimension = 1
        self.num_possible_states = self.grid_height * self.grid_width

        assert not self.pixel or not self.random_goal_place, 'pixel and random goal place not supported'
        ...
        self.trials = 100
        self._max_episode_steps = 1 * self.reward_for_achieving_goal
        self.id = "Four Rooms"
        self.level = level
        ...
        while not thing_placed:
            if self.level == 1:
                ...
                if random.random() > 0.5:
                    random_row = randint(int(self.grid_height / 2) + 1, self.grid_height - 1)
                    random_col = randint(0, self.grid_width - 1)
                else:
                    random_row = random_row = randint(0, self.grid_height - 1)
                    random_col = randint(int(self.grid_width / 2) + 1, self.grid_width - 1)
...
if __name__ == "__main__":
    grid = Four_Rooms_Static_Environment(15, 15, random_start_user_place=True, random_goal_place=False)
    grid.reset()
    grid.visualise_current_grid()
