...
class Base_Agent(object):
    ...
    def __init__(self, config, log_info=True, state_size=None):
        ...
        if hasattr(config, "min_steps_learn"):
            self.min_steps_learn = config.min_steps_learn
        else:
            self.min_steps_learn = self.hyperparameters.get("batch_size", 0)
        ...
        self.device = f"cuda:{config.GPU_id}" if config.use_GPU else "cpu"
        ...
        self.do_evaluation_iterations = self.hyperparameters.get("do_evaluation_iterations", False)
        self.training_episodes_per_eval_episode = self.hyperparameters.get(
            "training_episodes_per_eval_episode", TRAINING_EPISODES_PER_EVAL_EPISODE)
        self.state_actions_found = []

    def set_agent_round_and_tb(self, agent_round, tensorboard):
    ...
    def get_state_size(self):
        ...
        if isinstance(random_state, dict):
            ...
            return state_size
        elif len(self.environment.observation_space.shape) == 1:
            return self.environment.observation_space.shape[0]
        else:
            try:
                return torch.Size((self.environment.observation_space.shape[-1],) + self.environment.observation_space.shape[:-1])
            except:
    ...
    def get_score_required_to_win(self):
        ...
        if self.environment_title in ["AntMaze", "Hopper", "Walker2d"]:
            ...
            score = float("inf")
        try:
            score = self.environment.unwrapped.reward_threshold
        except AttributeError:
            try:
                score = self.environment.spec.reward_threshold
            except AttributeError:
                try:
                    score = self.environment.unwrapped.spec.reward_threshold
                except:
                    score = 0
        print("TITLE ", self.environment_title, "SCORE REQUIRED ", score)
        return score
    ...
    def run_n_episodes(self, num_episodes=None, show_whether_achieved_goal=True, save_and_print_results=True):
        ...
        if hasattr(self.config, 'num_steps_total'):
            return self.run_n_steps(self.config.num_steps_total, show_whether_achieved_goal=show_whether_achieved_goal, save_and_print_results=save_and_print_results)
    ...
    def run_n_steps(self, num_steps, show_whether_achieved_goal=True, save_and_print_results=True):
        """Runs n steps and then summarises results and saves model (if asked to)"""
        start = time.time()
    ...
    def evaluate(self, num_episodes=10):
        """Runs n episode on the game, and record score"""
        reward = 0
        for _ in range(num_episodes):
            self.reset_game()
            while not self.done:
                self.action = self.pick_action(True)
                reward += self.conduct_action_eval(self.action)
        self.total_episode_score_so_far = reward / num_episodes
        self.print_summary_of_latest_evaluation_episode()
        self.episode_number += 1

    def conduct_action_eval(self, action):
    ...
    def save_result(self):
        ...
        if self.episode_number == 1 or not self.do_evaluation_iterations:
            ...
            self.rolling_results.append(np.mean(self.game_full_episode_scores[-1 * self.rolling_score_window:]))
            self.save_max_result_seen()
            self.tensorboard.add_scalars('test_score',
                                         {f'{self.agent_name}_{self.agent_round}': self.total_episode_score_so_far},
                                         self.episode_number)
        elif (self.episode_number - 1) % self.training_episodes_per_eval_episode == 0:
            self.game_full_episode_scores.extend([self.total_episode_score_so_far for _ in range(self.training_episodes_per_eval_episode)])
            self.rolling_results.extend([np.mean(self.game_full_episode_scores[-1 * self.rolling_score_window:]) for _ in range(self.training_episodes_per_eval_episode)])
            self.save_max_result_seen()
            self.tensorboard.add_scalars('test_score',
                                         {f'{self.agent_name}_{self.agent_round}': self.total_episode_score_so_far},
                                         self.episode_number)
        else:
            self.tensorboard.add_scalars('training_epi_sco',
                                         {f'{self.agent_name}_{self.agent_round}': self.total_episode_score_so_far},
                                         self.episode_number)
    ...
    def print_rolling_result(self):
        ...
        text = """"\r Episode {0}, Steps {5}, Score: {3: .2f}, Max score seen: {4: .2f}, Rolling score: {1: .2f}, Max rolling score seen: {2: .2f}"""
    ...
    def enough_experiences_to_learn_from(self):
        ...
        return len(self.memory) > max(self.min_steps_learn, self.hyperparameters["batch_size"])
    ...
    def hard_update_of_target_network(self, local_model, target_model):
        """Updates the target network with the local network"""
        Base_Agent.copy_model_over(local_model, target_model)

    def create_NN(self, input_dim, output_dim, key_to_use=None, override_seed=None, hyperparameters=None):
        if isinstance(input_dim, tuple):
            return self.create_CNN(input_dim, output_dim, key_to_use=key_to_use,
                                  override_seed=override_seed, hyperparameters=hyperparameters)
        else:
            return self.create_FNN(input_dim, output_dim, key_to_use=key_to_use,
                                  override_seed=override_seed, hyperparameters=hyperparameters)

    def create_CNN(self, input_dim, output_dim, key_to_use=None, override_seed=None, hyperparameters=None):
        """Creates a cnn for the agents to use"""
        ...
        assert isinstance(hyperparameters["linear_hidden_units"][0], list), "Can't create a CNN with this linear_hidden_units"
        ...
        return CNN(input_dim=input_dim, layers_info=hyperparameters["linear_hidden_units"] + [['linear', output_dim]],
                   output_activation=hyperparameters["final_layer_activation"],
                   batch_norm=hyperparameters["batch_norm"], dropout=hyperparameters["dropout"],
                   hidden_activations=hyperparameters["hidden_activations"], initialiser=hyperparameters["initialiser"],
                   random_seed=seed).to(self.device)

    def create_FNN(self, input_dim, output_dim, key_to_use=None, override_seed=None, hyperparameters=None):
        """Creates a fully connected neural network for the agents to use"""
        if hyperparameters is None: hyperparameters = self.hyperparameters
        if key_to_use: hyperparameters = hyperparameters[key_to_use]
        if override_seed: seed = override_seed
        else: seed = self.config.seed
