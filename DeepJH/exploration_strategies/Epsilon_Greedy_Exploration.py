...
class Epsilon_Greedy_Exploration(Base_Exploration_Strategy):
    ...
    def __init__(self, config):
        ...
        self.eps_min = config.eps_min if hasattr(
            config, "eps_min") else 0.01
        self.eps_max = config.eps_max if hasattr(
            config, "eps_max") else 1
        self.eps_decay = config.eps_decay if hasattr(
            config, "eps_decay") else 1000000
        self.min_steps_learn = config.min_steps_learn if hasattr(
            config, "min_steps_learn") else 1
        self.nature = config.nature if hasattr(config, 'nature') else False

        if "exploration_cycle_episodes_length" in self.config.hyperparameters.keys() and \
                self.config.hyperparameters["exploration_cycle_episodes_length"]:
    ...
    def get_updated_epsilon_exploration(self, action_info, epsilon=1.0):
        ...
        if self.nature:
            ...
            prog = min(1, max(0, steps - self.min_steps_learn) /
                       (self.eps_decay - self.min_steps_learn))
            epsilon = prog * self.eps_min + (1 - prog) * self.eps_max
        else:
            episode_number = action_info["episode_number"]
