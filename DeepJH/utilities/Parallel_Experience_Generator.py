...
class Parallel_Experience_Generator(object):
    ...
    def __init__(self, environment, policy, seed, hyperparameters, action_size, device, action_choice_output_columns=None):
        self.device = device
    ...
    def play_n_episodes(self, n, exploration_epsilon=None, greedy=False):
    ...
    def __call__(self, n):
        ...
        return self.play_1_episode(exploration, greedy=self.greedy)

    def play_1_episode(self, epsilon_exploration, greedy=False):
        ...
        episode_rewards = []
        while not done:
            action = self.pick_action(self.policy, state, epsilon_exploration, greedy=greedy)
    ...
    def pick_action(self, policy, state, epsilon_exploration=None, greedy=False):
